...
import { get_direct_edge, get_direct_edges, is_parent_table } from '../../helpers/schema_helpers'
import { OrmaSchema } from '../../schema/schema_types'
import { is_submutation, path_to_table } from './mutate_helpers'
...
export const get_foreign_keys_in_mutation = (
    ...
    const table_name = path_to_table(record_path)
    ...
    const foreign_keys = all_paths.flatMap(parent_path => {
        const parent_table_name = parent_path?.[parent_path.length - 2]
        // dont do anything for the child paths (foreign keys only come from parents by definition)
        if (!is_parent_table(parent_table_name, table_name, orma_schema)) {
            return []
        }
