...
export const compile_definition = ({
    statement,
    database_type
}: DDLCompilerArgs<Definition>) => {
    if ('data_type' in statement) {
        return compile_column_definition({ statement, database_type })
    }
    if ('index' in statement) {
        return compile_index_definition({ statement, database_type })
    }
