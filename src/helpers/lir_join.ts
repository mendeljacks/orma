 ...
 */
 export const lir_join = (left_list, inner_list, right_list, left_fn, inner_fn, right_fn) => {

    // Safety check called once per invocation
    if (!Array.isArray(left_list)) throw new Error('Left list must be an array')
    if (!Array.isArray(right_list)) throw new Error('Right list must be an array')
    if (!(left_fn instanceof Function)) throw new Error('Left function must be a function')
    if (!(right_fn instanceof Function)) throw new Error('Right function must be a function')
    if (!(inner_fn instanceof Function)) throw new Error('Inner function must be a function')
    ...
    for (let i = 0; i < left_list.length; i++) {
        ...
        if (!memory[left_list_val]) memory[left_list_val] = { left: [], right: [] }
    ...
    for (let i = 0; i < right_list.length; i++) {
        ...
        if (!memory[right_list_val]) memory[right_list_val] = { left: [], right: [] }
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (memory[key].left.length > 0 && memory[key].right.length > 0) {
            output.inner = inner_fn(memory[key].left, output.inner, memory[key].right)
        }
        if (memory[key].left.length === 0 && memory[key].right.length > 0) {
